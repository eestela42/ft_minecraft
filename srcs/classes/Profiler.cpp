...
void Profiler::LogData()
{
	...
	if (rc != 0)
		assert(!"LMDB env create didn't work");
	rc = mdb_env_open(env, DB_PATH, 0, 0664);
	if (rc != 0)
		assert(!"LMDB env open didn't work");
	rc = mdb_txn_begin(env, NULL, 0, &txn);
	if (rc != 0)
		assert(!"LMDB transaction begin didn't work");
	rc = mdb_dbi_open(txn, 0, 0, &dbi);
	if (rc != 0)
		assert(!"LMDB database open didn't work");
	mdb_txn_abort(txn);
...
void Profiler::InsertData(const char *keyName, const char *keyType, const char *value)
{
	...
	if (rc != 0)
		assert(!"LMDB insert transaction begin didn't work");
	...
	if (rc != 0)
		assert(!"LMDB insert transaction commit didn't work");
}
...
char *Profiler::RetrieveData(const char *keyName, const char *keyType)
{
	...
	if (rc != 0)
		assert(!"LMDB retrieve transaction begin didn't work");
