...
void ChunkRLE::makeTopBotFaces(std::vector<std::vector<std::vector<u_int>>> &tool, u_char *ruban, u_int lowest, u_char *map)
{
	...
	for (u_int y = 0; y < sizeY; y++)
	{
		...
		{
			// algo de face making
			if (map[x + y * AChunk::sizeX] != 0 || !tool[x][y][face_type] || tool[x][y][face_type] == 4 && tool[x][y][height] != lowest || tool[x][y][face_type] == 5 && tool[x][y][height] + ruban[tool[x][y][ruban_pos] + 1] - 1 != lowest)
				continue;
			...
			{
				if (map[px + y * AChunk::sizeX] != 0 || tool[px][y][height] != lowest || tool[x][y][face_type] != tool[px][y][face_type] || tool[x][y][block_type] != tool[px][y][block_type] || tool[px][y][height] != tool[x][y][height])
					break;
				longX++;
			...
			for (u_int py = y + 1; py < sizeY; py++)
			{
				...
				{
					if (map[px + py * AChunk::sizeX] != 0 || tool[px][py][height] != lowest || tool[x][y][face_type] != tool[px][py][face_type] || tool[x][y][block_type] != tool[px][py][block_type] || tool[px][py][height] != tool[x][y][height])
						break;
					tmpX++;
				}
				if (tmpX != longX)
