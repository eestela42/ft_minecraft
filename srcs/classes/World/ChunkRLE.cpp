...
void 		ChunkRLE::makeTopBotFaces(std::vector<std::vector<std::vector<u_int>>> &tool, u_char *ruban, u_int lowest, u_char *map)
{
	...
	for (u_int x = 0; x < sizeX; x++)
	{
		...
		// algo de face making
		if (map[x + y * Chunk::sizeX] != 0 || !tool[x][y][t_face_type]
			|| tool[x][y][t_face_type] == 4 && tool[x][y][t_height] != lowest
			|| tool[x][y][t_face_type] == 5 && tool[x][y][t_height] + ruban[tool[x][y][t_ruban_pos] + 1] - 1 != lowest)
			continue ;
		...
		for (u_int px = x + 1; px < sizeX; px++)
		{
			if (map[px + y * Chunk::sizeX] != 0 || tool[px][y][t_height] != lowest || tool[x][y][t_face_type] !=  tool[px][y][t_face_type]
				||  tool[x][y][t_block_type]  !=  tool[px][y][t_block_type] || tool[px][y][t_height] != tool[x][y][height])
				break ;
			longX++;
		...
		for (u_int py = y + 1; py < sizeY; py++)
		{
			...
			{
				if (map[px + py * Chunk::sizeX] != 0 || tool[px][py][t_height] != lowest || tool[x][y][t_face_type] !=  tool[px][py][t_face_type]
					||  tool[x][y][t_block_type]  !=  tool[px][py][t_block_type]  || tool[px][py][t_height] != tool[x][y][t_height])
					break ;
				tmpX++;
			}
			if (tmpX != longX)
